// Water Button — Originkit
// Converted from TSX to plain JSX for this project (types stripped, logic unchanged).
//
// Example props:
//   label: "Signup"
//   font: {"fontSize":"24px","textAlign":"left","fontFamily":"Inter","fontWeight":700,"lineHeight":"1.2em","letterSpacing":"-0.01em"}
//   paddingX: 100
//   paddingY: 28
//   waterAmount: 15
...
const dropShadow = (color, intensity) => {
  ...
  const tone = (alpha) => `rgba(${r}, ${g}, ${b}, ${(alpha * k * a).toFixed(4)})`;
  return [
    `0 0 0.75px ${tone(0.2)}`,
...
export default function WaterButton(props) {
  ...
  const borderStroke = Math.max(0, borderOptions?.stroke ?? DEFAULTS.borderOptions.stroke);
  const shadowColor = shadowOptions?.color ?? DEFAULTS.shadowOptions.color;
  const shadowIntensity = shadowOptions?.intensity ?? DEFAULTS.shadowOptions.intensity;
  ...
  useEffect(() => {
    ...
    function draw() {
      ...
      roundRectPath(ctx, half, half, W - borderStroke, H - borderStroke, rounded - half);
  ...
  }, [waterAmount, waterColor, border, borderColor, borderStroke, paddingX, paddingY, rounded, label]);
